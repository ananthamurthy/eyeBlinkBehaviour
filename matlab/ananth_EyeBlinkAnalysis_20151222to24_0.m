...
% SPECIAL NOTE - This version of the code will work for data obtained from
% 20151222-24
...
while 1
    ...
    for trialNum = 1:totalTrials
        ...
        rawData_size(trialNum) = size(csvread(filename, 0, 0), 1);
    end
    ...
    for trialNum = 1:totalTrials
        filename = [direc, 'Trial', num2str(trialNum), '.csv'];
        csType(trialNum)= csvread(filename, 0, 2, [ 0 2 0 2] ); %reads only row 1, column 3
    ...
    if saveData == 1
        ...
        save([saveDirec dataset '_csMinus_fullFiltered'], 'blinkData_csMinus_fullFiltered');
        save([saveFolder, dataset, '_csPlus_fullFiltered_trialAvg.csv'], 'blinkData_csPlus_fullFiltered_trialAvg');
        save([saveFolder, dataset, '_csMinus_fullFiltered_trialAvg.csv'], 'blinkData_csMinus_fullFiltered_trialAvg');
        %raw data
        ...
        if findSignificantBlinks == 1
            %scores
            %CS+
            save([saveFolder, dataset, '_csPlus_hitScore.csv'], 'csPlus_hitScore');
            save([saveFolder, dataset, '_csPlus_missScore.csv'], 'csPlus_missScore');
            save([saveFolder, dataset, '_csPlus_fullScore.csv'], 'csPlus_fullScore');
            %CS-
            save([saveFolder, dataset, '_csMinus_hitScore.csv'], 'csMinus_hitScore');
            save([saveFolder, dataset, '_csMinus_missScore.csv'], 'csMinus_missScore');
            save([saveFolder, dataset, '_csMinus_fullScore.csv'], 'csMinus_fullScore');
            
            %blink rastors
            %CS+
            save([saveFolder, dataset, '_blinks_csPlus_fullTrial.csv'], 'blinks_csPlus_fullTrial');
            save([saveFolder, dataset, '_blinks_csPlus_pretone.csv'], 'blinks_csPlus_pretone');
            save([saveFolder, dataset, '_blinks_csPlus_cs.csv'], 'blinks_csPlus_cs');
            %CS-
            save([saveFolder, dataset, '_blinks_csMinus_fullTrial.csv'], 'blinks_csPlus_fullTrial');
            save([saveFolder, dataset, '_blinks_csMinus_pretone.csv'], 'blinks_csMinus_pretone');
            save([saveFolder, dataset, '_blinks_csMinus_cs.csv'], 'blinks_csMinus_cs');
        end
    end
end
pause(5);
